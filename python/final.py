...
#		self.canvas.create_text(405,410,fill="white",font="arial 10", text="0")
	...
	def animation(self):
		...
		avail = []
		colors = ['red', 'orange', 'blue', 'pink', 'yellow']
		while True:
			...
			if len(data) == 4:
				...
				dist1 = int(data[2])
				dist2 = int(data[3])
				with open("data.json") as fle:
					ld = json.load(fle)
				msg = {'name': str(name),'datetime' : str(dt.now()), 'angle' : angle, 'dist1' : dist1, 'dist2' : dist2}
				ld.insert(0, msg)
				with open("data.json", 'w+') as fle:
					fle.write(json.dumps(ld))
...
#				self.canvas.create_oval(x1-5,y1-5,x1+5,y1+5, fill="orange")
				self.obj1 = self.canvas.create_oval(x1-10,y1-10,x1+10,y1+10, fill=colors[avail.index(name)-1])
...
#				self.canvas.create_oval(x2-5,y2-5,x2+5,y2+5, fill="orange")
				if dist2 > 0:
					self.obj2 = self.canvas.create_oval(x2-10,y2-10,x2+10,y2+10, fill=colors[avail.index(name)-1])
			self.canvas.update()
...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
	...
	def do_GET(self):
		...
		self.end_headers()
		# data = "rajacar*145*17*85"
		with open('temp.txt', 'r') as fle:
			data = fle.read()
...
def runserver():
	...
	server = HTTPServer(('10.0.51.21', port), SimpleHTTPRequestHandler)
